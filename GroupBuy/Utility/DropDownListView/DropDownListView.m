...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self)
    {
        ...
        for (int i = 0; i <sectionNum; i++)
        {
          ...
          //  sectionBtn.backgroundColor = C6;
            sectionBtn.titleLabel.font = [UIFont systemFontOfSize:14.0f];
            [self addSubview:sectionBtn];
            
            UIImageView *sectionBtnIv = [[UIImageView alloc] initWithFrame:CGRectMake(sectionWidth*i +(sectionWidth - 16), (self.frame.size.height-8)/2, 10, 6)];
            [sectionBtnIv setImage:[UIImage imageNamed:@"pullDownArrow.png"]];
            ...
            if ([self.dropDownDataSource respondsToSelector:@selector(iconInSection:)])
            {
                UIImageView *icon = [[UIImageView alloc] initWithFrame:CGRectMake(sectionWidth*i+10,(self.frame.size.height-15)/2.0, 14, 14)];
                icon.image = [self.dropDownDataSource iconInSection:i];
...
-(void)showChooseListViewInSection:(NSInteger)section
                      choosedIndex:(NSInteger)index
{
  //  NSLog(@"section = %d",section);
    NSInteger row = [self.dropDownDataSource numberOfRowsInSection:section];
    //NSLog(@"row = %d",row);
    tableViewHeight = row*CELL_HEIGHT;
    ...
    if (!self.mTableView) {
        ...
        self.mTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, tableViewHeight) style:UITableViewStylePlain];
        self.mTableView.backgroundColor = self.backgroundColor;
