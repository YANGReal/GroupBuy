                     ...
                     space:(CGFloat)s
{
	...
	[s2 setFrame:CGRectMake(width+space,     0, width, height)];
	[s3 setFrame:CGRectMake(2 * width+space*2, 0, width, height)];
	[s4 setFrame:CGRectMake(3 * width+space*3, 0, width, height)];
	[s5 setFrame:CGRectMake(4 * width+space*4, 0, width, height)];
...
-(void)displayRating:(float)rating
{
    ...
    [viewDelegate ratingView:self DidChangeRating:rating];
}
...
-(void) touchesMoved: (NSSet *)touches withEvent: (UIEvent *)event
{
	...
	int newRating = (int) (pt.x / (width+space)) + 1;
